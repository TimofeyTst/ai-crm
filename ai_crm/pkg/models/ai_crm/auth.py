...
class RegisterRequest(base_models.BaseModel):
    username: str = Field(description="Username", max_length=50, example="johndoe")
    email: EmailStr = Field(description="User email address", example="user@example.com")
    password: str = Field(description="User password", min_length=3, example="password123")
    first_name: str | None = Field(None, description="User first name", example="John")
    last_name: str | None = Field(None, description="User last name", example="Doe")
