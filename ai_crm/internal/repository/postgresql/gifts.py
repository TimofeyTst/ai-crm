...
async def get_gift_by_id(context: context.AnyContext, gift_id: int) -> gift_models.Gift:
    async with psql.get_connection(context, read_only=True) as conn:
        rows = await conn.fetch("SELECT * FROM gifts WHERE gift_id = $1", gift_id)
        if not rows or len(rows) != 1:
            raise psql_exceptions.EmptyResult(f"Gift with id {gift_id} not found")
...
async def update_gift(context: context.AnyContext, request: gift_models.GiftUpdateRequest) -> gift_models.Gift:
    async with psql.get_connection(context) as conn:
        ...
        if not row:
            raise psql_exceptions.EmptyResult(f"Gift with id {request.gift_id} not found")
        return row

async def delete_gift(context: context.AnyContext, request: gift_models.GiftDeleteRequest) -> None:
    async with psql.get_connection(context) as conn:
        result = await conn.execute("DELETE FROM gifts WHERE gift_id = $1", request.gift_id)
