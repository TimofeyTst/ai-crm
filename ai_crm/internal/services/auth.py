...
from ai_crm.pkg.models.exceptions import auth as auth_exceptions, postgres as postgres_exceptons, users as users_excpetions
from ai_crm.pkg.utils import jwt as jwt_utils, password as password_utils
...
async def register(
    ...
    logger.info(f"Password hashed successfully for username: {request.username}, hash length: {len(hashed_password)}")
...
async def refresh(
    ...
    user = await _get_user_from_jwt(context, refresh_token, token_type="refresh")
    return _create_tokens(user)
